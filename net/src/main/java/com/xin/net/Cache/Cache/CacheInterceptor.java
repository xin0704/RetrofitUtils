package com.xin.net.Cache.Cache;
...
public class CacheInterceptor implements Interceptor
{
            ...
            try
            {
                ...
                if (response.isSuccessful()) // 只有在网络请求返回成功之后，才进行缓存处理，否则，404存进缓存，岂不笑话
                {
                    ...
                    if (responseBody != null)
                    {
                        ...
                        Logger.i("HttpRetrofit", "--> Push Cache:" + url + " :Success");
                    }
    ...
    {
        Logger.i("HttpRetrofit", "--> Try to Get Cache   --------");
        String url = request.url().url().toString();
        ...
        if (cacheStr == null)
        {
            Logger.i("HttpRetrofit", "<-- Get Cache Failure ---------");
            return null;
        }
        ...
        Logger.i("HttpRetrofit", "<-- Get Cache: " + response.code() + " " + response.message() + " " + url + " (" + useTime + "ms)");
        Logger.i("HttpRetrofit", cacheStr + "");
        return response;
    }
